...
namespace GSMA.MobileConnect.Test
{
    ...
    public class MobileConnectWebInterfaceMockTests
    {
        ...
        public async Task RefreshTokenShouldUseRequestTokenUrlWhenNoRefreshUrl()
        {
            ...
            _restClient.QueueParallelResponses(Tuple.Create<string, object>(discoveryResponse.ProviderMetadata.RefreshEndpoint, _responses["error"]),
                Tuple.Create<string, object>(discoveryResponse.ProviderMetadata.TokenEndpoint, _responses["token"]));
        ...
        public async Task RevokeTokenShouldReturnOk()
        {
            string session = "zzzz";
            var discoveryResponse = CompleteDiscovery();
            discoveryResponse.ProviderMetadata.RevokeEndpoint = "http://revoke";
            await _cache.Add(session, discoveryResponse);

            _restClient.QueueResponse(_responses["token-revoked"]);

            var result = await _mobileConnect.RevokeTokenAsync(_request, "token", null, session);
        ...
        [Test]
        public async Task RevokeTokenShouldReturnErrorIfNoUrlAvailable()
        {
            string session = "zzzz";
            var discoveryResponse = CompleteDiscovery();
            discoveryResponse.ProviderMetadata.RevokeEndpoint = null;
            discoveryResponse.ResponseData?.response?.apis?.operatorid?.link?.Clear();
            await _cache.Add(session, _discoveryResponse);
